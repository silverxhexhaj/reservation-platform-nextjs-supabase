...
export const BusinessCard: React.FC<BusinessCardProps> = ({ business }) => {
    ...
    return (
        <motion.div variants={itemVariants}>
            ...
            >
                <Card className={`overflow-hidden h-full transition-all duration-300 hover:shadow-lg border bg-white ${business.isPremium ? 'border-amber-200 hover:border-amber-300' : 'border-gray-100 hover:border-gray-200'
                    }`}>
                        ...
                        <div className="flex items-center justify-between mt-3 pt-3">
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center space-x-1 text-neutral-500">
                                    ...
                                    <span className="text-xs">{business.reviewCount ?? 0}</span>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center space-x-0.5">
                                {Array.from(business.priceRange ?? '').map((_, index) => (
                                    <span
                                        key={index}
                                        className={`text-[10px] font-medium ${business.isPremium ? 'text-amber-600' : 'text-neutral-900'
                                            }`}
                                    >
                                        $
                                    </span>
